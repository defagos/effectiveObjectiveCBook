...
\section{Know the conventions governing memory management}\section{sec:refCountingConventions}
12
23
29
...
Since autorelease and NSAutoreleasePool are not part of the Objective-C runtime but belong to the fundamental framework, pool management is the responsibility of the programmer. You are not limited to the use of only one autorelease pool. Each thread namely manages its own pool stack, with the most recently created pool on top of it. But at least you should ensure that a pool is created when a thread starts, and gets destroyed when the thread exits. This holds for the main thread as well.\footnote{If you are using Xcode, project templates generate the corresponding code for you.} 

The use of several autorelease pools per thread provides for a finer-grained control of memory usage, especially when many autoreleased objects are created locally.\footnote{Frameworks usually introduce use additional autorelease pools. For example, the iOS UIKit framework creates an autorelease pool when a run loop begins, and releases it when the run loop ends.} When an autorelease message is sent to an object within a given thread, the object is nameley registered with the pool on top of the stack. By enclosing code which locally generates many autoreleased objects within a pool creation / destruction pair, you can reduce the duration of the associated memory footprint.

It is important that a function or a class which creates an autorelease pool also releases it when it is done with it. Failure to do so would break the rule that you can use autoreleased objects returned from functions. Imagine for example



The autorelease message is mainly used in the following cases:
\begin{itemize}
\item When an object created within a function is returned to the caller, but that the caller must not be responsible of performing the corresponding release. By sending an autorelease message to the object it created before returning it, the function guarantees its caller that it can still use the object after the function has returned. This is for example the case with factory methods:
\begin{lstlisting}[frame=single]
- (void)method
{
    Movie *movie = [Movie movieWithTitle:@"Scarface"];
    // Can use movie safely
    [movie printActors];
    // ...
}
\end{lstlisting}
\item For creating temporary objects. Such objects can be either left dangling until they die, or immediately retained:
- (void)someMethod
{
    // The movie object is then left dangling until the pool it is registered with is cleared
    Movie *movie = [[[Movie alloc] initWithTitle:@"The Godfather 1"] autorelease];
    [movie printActors];
    
    // The otherMovie object is here immediately retained (assume that the currentlyPlayedMovie
    // property has the retain attribute)
    m_cinema.currentlyPlayedMovie = movie;
}
\end{itemize}

Temporary objects: Two cases:
a) Temporary object




example: loop with temporary objects -> no need for autorelease, simply alloc / release. Exception: if function gets called to create the objects, it may be useful.
...
30. Autorelease pool: If you manage, be sure to create and destroy in the same scope, otherwise you cannot trust autoreleased objects returned from functions at all, in contradiction to conventions
