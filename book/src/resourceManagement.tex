...
\item autorelease: In C++, it is safe (and considered good practice) to return a reference-counted pointer to a resource from a function which has acquired it. No such idiom can exist in Objective-C without support from the runtime, and a solution had to be found: The autorelease message. By sending such a  message, you ask the runtime not to call dealloc when the reference counter drops to zero, but to put the object within an object pool for some more time, without having the object sent the dealloc message. During the time the autoreleased object remains in the pool, somebody might reclaim it, bringing the object to life again. The exact time at which the object will be released is handled by the concept of autorelease pools, waiting rooms for autoreleased objects ready for deallocation. Autorelease pool creation is the responsibility of the programmer, not of the runtime. Many pools can be created within a given thread, in which case they will be arranged as a stack. An object which has been autoreleased once in its lifetime ends up in the top pool when its counter drops to zero. The first thing you should do when forking a new thread is to create an associated autorelease pool, otherwise you won't be able to call autorelease from it. This of course holds for the main thread as well.\footnote{If you are using Xcode, project templates usually generate the corresponding code for you} Conversely, the last thing you should do when leaving a thread is releasing this memory pool. Additional pools are optional and can be used to get a fine-grained control over the lifetime of autoreleased objects. Note that pools are in general also silently managed by frameworks you use. For example, the iOS UIKit framework creates an autorelease pool when a run loop begins, and releases it when the run loop ends. In general, you should therefore assume that an autoreleased object only resides in a pool for a small amount of time. From a programmer's point of view, you can in general safely assume that an autoreleased object can still be used within the current block. Objects received from functions are generally autoreleased, and you can also safely assume they are still valid since the time they were created and autoreleased, even if that happened deeply nested within a function call chain (see \ref{sec:autoreleasePools}).
\item copy
\end{itemize}
